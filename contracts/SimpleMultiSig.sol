...
contract SimpleMultiSig {
    ...
    constructor(uint threshold_, address[] owners_, uint chainId) public {
        ...
        require(threshold_ > 0, "threshold should be larger than 0");
    ...
    function execute(uint8[] sigV, bytes32[] sigR, bytes32[] sigS, address destination, uint value, bytes data, address executor, uint gasLimit) public {
        ...
        require(executor == msg.sender || executor == address(0), "need executor to be address(0)");
        ...
        for (uint i = 0; i < threshold; i++) {
            ...
            require(recovered > lastAdd && isOwner[recovered], "recovered address should be owner");
            lastAdd = recovered;
        }
